...
Single-file Streamlit app (`app.py`, ~1400 lines) structured in 6 sequential sections:

1. **TRANSLATIONS** — Bilingual dictionary (EN/TH) with ~350 keys. Every UI string goes through `t(key)` helper.
...
2. **Subject** — gender, age, ethnicity, body type, appearance, hair, expression
3. **Outfit** — fashion presets + free text + fabric + color palette + accessories
4. **Pose** — selected pose
5. **Environment** — location + time of day + season + weather effects
6. **Camera & Lighting** — lighting style + shot framing + camera angle + DOF + picture style
